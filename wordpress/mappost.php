...
<div class='container_wrap container_wrap_first main_color <?php avia_layout_class( 'main' ); ?>'>
    ...
    <div class='container'>
        ...
        <main class='template-page content  <?php avia_layout_class( 'content' ); ?> units' <?php avia_markup_helper(array('context' => 'content','post_type'=>'page'));?>>
            ...
            <p><?php echo apply_filters( 'tooltip_shortcodes_filter', get_field('shortDesc')); ?><p>
                    <?php endif; ?>

                    <?php if( get_field('longDesc') ): ?>
                        <?php echo apply_filters( 'tooltip_shortcodes_filter', get_field('longDesc')); ?>
                    <?php endif; ?>

                    <?php if( get_field('currentSituation') ): ?>
                <h2><?php if (function_exists('pll_e')) { pll_e( 'Stand der Umbenennung' ); } ?></h2>
            <p><?php echo apply_filters( 'tooltip_shortcodes_filter', get_field('currentSituation')); ?><p>
                    <?php endif; ?>

                    <?php if( get_field('recommendation') ): ?>
                <h2><?php if (function_exists('pll_e')) { pll_e( 'Unsere Empfehlung' ); } ?></h2>
            <p><?php echo apply_filters( 'tooltip_shortcodes_filter', get_field('recommendation')); ?><p>
                    <?php endif; ?>
...
<script type="application/javascript">
    ...
    const positionTooltip = (tooltip) => {
        ...
        tooltiptext.style.left = `${
            Math.min(
                Math.max(-tooltip.offsetLeft, -(tooltiptext.offsetWidth / 2)),
                main.offsetWidth - tooltip.offsetLeft)
        }px`;
    }
